...
const NotesPage = () => {
  ...
  const fetchNotes = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('https://note-app-backend-31cy.onrender.com/api/notes', {
  ...
  const handleAddNote = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('https://note-app-backend-31cy.onrender.com/api/notes', newNote, {
        headers: { Authorization: token },
      });
  ...
  const handleNoteChange = async (index, key, value) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://note-app-backend-31cy.onrender.com/api/notes/${updatedNotes[index]._id}`, updatedNotes[index], {
        headers: { Authorization: token },
      });
    } catch (error) {
  ...
  const handleDeleteNote = async (index) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://note-app-backend-31cy.onrender.com/api/notes/${notes[index]._id}`, {
        headers: { Authorization: token }
      });
